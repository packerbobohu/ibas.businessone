...
/** 列表应用-公司 */
export class CompanyListApp extends ibas.BOListApplication<ICompanyListView, bo.Company> {

    /** 应用标识 */
    static APPLICATION_ID: string = "7d461fcc-b0ca-4e29-8c31-0900fe9609a9";
    /** 应用名称 */
    static APPLICATION_NAME: string = "businessone_app_company_list";
    /** 业务对象编码 */
    ...
    /** 查看数据，参数：目标数据 */
    protected viewData(data: bo.Company): void {
        // 检查目标数据
    ...
    /** 编辑数据，参数：目标数据 */
    protected editData(data: bo.Company): void {
        // 检查目标数据
        if (ibas.objects.isNull(data)) {
            this.messages(ibas.emMessageType.WARNING, ibas.i18n.prop("shell_please_chooose_data",
    ...
    /** 删除数据，参数：目标数据集合 */
    protected deleteData(data: bo.Company): void {
        // 检查目标数据
        if (ibas.objects.isNull(data)) {
            this.messages(ibas.emMessageType.WARNING, ibas.i18n.prop("shell_please_chooose_data",
...
/** 视图-公司 */
export interface ICompanyListView extends ibas.IBOListView {
    /** 编辑数据事件，参数：编辑对象 */
